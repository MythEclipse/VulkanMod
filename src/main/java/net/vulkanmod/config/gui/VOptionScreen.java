...
public class VOptionScreen extends net.minecraft.client.gui.screens.Screen {
    ...
    public VOptionScreen(
        ...
        super(title);
        this.ICON = net.minecraft.resources.Identifier.fromNamespaceAndPath(
    ...
    private void addPages() {
        ...
        net.vulkanmod.config.option.OptionPage page = new net.vulkanmod.config.option.OptionPage(
                net.minecraft.network.chat.Component.translatable(
                        "vulkanmod.options.pages.video")
                        .getString(),
                net.vulkanmod.config.option.Options.getVideoOpts());
        this.optionPages.add(page);
        net.vulkanmod.config.option.OptionPage page2 = new net.vulkanmod.config.option.OptionPage(
                net.minecraft.network.chat.Component.translatable(
                        "vulkanmod.options.pages.graphics")
                        .getString(),
                net.vulkanmod.config.option.Options.getGraphicsOpts());
        this.optionPages.add(page2);
        net.vulkanmod.config.option.OptionPage page3 = new net.vulkanmod.config.option.OptionPage(
                net.minecraft.network.chat.Component.translatable(
                        "vulkanmod.options.pages.optimizations")
                        .getString(),
                net.vulkanmod.config.option.Options.getOptimizationOpts());
        this.optionPages.add(page3);
        net.vulkanmod.config.option.OptionPage page4 = new net.vulkanmod.config.option.OptionPage(
                net.minecraft.network.chat.Component.translatable(
    ...
    protected void init() {
        addPages();
        int listWidth = java.lang.Math.min(
    ...
    private void addPageButtons(int x0, int y0, int width, int height, boolean verticalLayout) {
        ...
        for (int i = 0; i < this.optionPages.size(); i++) {
            ...
            int finalIdx = i;
            net.vulkanmod.config.gui.widget.VButtonWidget widget = new net.vulkanmod.config.gui.widget.VButtonWidget(
                    x,
                    y,
                    width,
                    height,
    ...
    private void addButtons() {
        int buttonWidth = this.minecraft.font.width(net.minecraft.network.chat.CommonComponents.GUI_DONE)
                + (2 * 10);
        int x0 = (this.width - buttonWidth) - 20;
        int y0 = (this.height - 20) - 7;
        this.doneButton = new net.vulkanmod.config.gui.widget.VButtonWidget(
                x0,
                y0,
                buttonWidth,
                20,
        ...
        int buttonWidth2 = this.minecraft.font.width(
                net.minecraft.network.chat.Component.translatable(
                        "vulkanmod.options.buttons.apply"))
                + (2 * 10);
        this.applyButton = new net.vulkanmod.config.gui.widget.VButtonWidget(
                x0 - (buttonWidth2 + 5),
                y0,
                buttonWidth2,
                20,
        ...
        int buttonWidth3 = this.minecraft.font.width(
                net.minecraft.network.chat.Component.translatable(
                        "vulkanmod.options.buttons.kofi"))
                + 10;
        int x02 = (this.width - buttonWidth3) - 20;
        this.supportButton = new net.vulkanmod.config.gui.widget.VButtonWidget(
                x02,
                6,
                buttonWidth3,
                20,
        ...
        if (net.vulkanmod.config.UpdateChecker.isUpdateAvailable()) {
            int buttonWidth4 = this.minecraft.font.width(
                    net.minecraft.network.chat.Component.translatable(
                            "vulkanmod.options.buttons.update_available"))
                    + 10;
            net.vulkanmod.config.gui.widget.VButtonWidget updateButton = new net.vulkanmod.config.gui.widget.VButtonWidget(
                    (x02 - buttonWidth4) - 5,
                    6,
                    buttonWidth4,
                    20,
    ...
    public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent event, boolean bl) {
        for (net.minecraft.client.gui.components.events.GuiEventListener element : children()) {
            if (element != null && element.mouseClicked(event, bl)) {
                setFocused(element);
                if (event.button() == 0) {
                    setDragging(true);
                }
    ...
    public boolean mouseReleased(net.minecraft.client.input.MouseButtonEvent event) {
        ...
        return getChildAt(event.x(), event.y())
                .filter(
                        guiEventListener -> {
                            return guiEventListener != null && guiEventListener.mouseReleased(event);
                        })
                .isPresent();
    }
    ...
    public void render(
        ...
        java.util.List<net.minecraft.util.FormattedCharSequence> list = getHoveredButtonTooltip(currentList, mouseX,
                mouseY);
        if (list != null) {
