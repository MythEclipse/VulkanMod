...
public abstract class VTextureSelector {
    ...
    private static final net.vulkanmod.vulkan.texture.VulkanImage[] boundTextures = new net.vulkanmod.vulkan.texture.VulkanImage[12];
    private static final int[] levels = new int[12];
    private static final net.vulkanmod.vulkan.texture.VulkanImage whiteTexture = net.vulkanmod.vulkan.texture.VulkanImage.createWhiteTexture();
    ...
    public static void bindTexture(int i, net.vulkanmod.vulkan.texture.VulkanImage texture) {
        if (i < 0 || i >= 12) {
            net.vulkanmod.Initializer.LOGGER.error(java.lang.String.format("On Texture binding: index %d out of range [0, %d]", java.lang.Integer.valueOf(i), 11));
        } else {
    ...
    public static void bindImage(int i, net.vulkanmod.vulkan.texture.VulkanImage texture, int level) {
        if (i < 0 || i > 7) {
            net.vulkanmod.Initializer.LOGGER.error(java.lang.String.format("On Texture binding: index %d out of range [0, %d]", java.lang.Integer.valueOf(i), 11));
        } else {
            boundTextures[i] = texture;
    ...
    public static void uploadSubTexture(int mipLevel, int width, int height, int xOffset, int yOffset, int unpackSkipRows, int unpackSkipPixels, int unpackRowLength, java.nio.ByteBuffer buffer) {
        uploadSubTexture(mipLevel, 0, width, height, xOffset, yOffset, unpackSkipRows, unpackSkipPixels, unpackRowLength, org.lwjgl.system.MemoryUtil.memAddress(buffer));
    }
    ...
    public static void bindShaderTextures(net.vulkanmod.vulkan.shader.Pipeline pipeline) {
        java.util.List<net.vulkanmod.vulkan.shader.descriptor.ImageDescriptor> imageDescriptors = pipeline.getImageDescriptors();
        for (net.vulkanmod.vulkan.shader.descriptor.ImageDescriptor state : imageDescriptors) {
            com.mojang.blaze3d.textures.GpuTextureView textureView = net.vulkanmod.vulkan.VRenderSystem.getShaderTexture(state.imageIdx);
            if (textureView != null) {
                net.vulkanmod.render.engine.VkGpuTexture gpuTexture = (net.vulkanmod.render.engine.VkGpuTexture) textureView.texture();
                int shaderTexture = gpuTexture.glId();
                net.vulkanmod.gl.VkGlTexture texture = net.vulkanmod.gl.VkGlTexture.getTexture(shaderTexture);
    ...
    public static void setActiveTexture(int activeTexture2) {
        if (activeTexture2 < 0 || activeTexture2 >= 12) {
            net.vulkanmod.Initializer.LOGGER.error(java.lang.String.format("On Texture binding: index %d out of range [0, %d]", java.lang.Integer.valueOf(activeTexture2), 11));
        }
