...
public class SwapChain extends Framebuffer {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            if (extent.width() == 0 && extent.height() == 0) {
                if (this.swapChainId != VK_NULL_HANDLE) {
                    this.swapChainImages.forEach(
                            image -> vkDestroyImageView(device, image.getImageView(), null));
    ...
    private static VkExtent2D getExtent(VkSurfaceCapabilitiesKHR capabilities) {
        ...
        actualExtent.width(
                MathUtil.clamp(minExtent.width(), maxExtent.width(), actualExtent.width()));
        actualExtent.height(
                MathUtil.clamp(minExtent.height(), maxExtent.height(), actualExtent.height()));
    ...
    private static int checkPresentMode(int... requestedModes) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            return VK_PRESENT_MODE_FIFO_KHR; // If None of the request modes exist/are supported by
            // Driver
        }
    }
