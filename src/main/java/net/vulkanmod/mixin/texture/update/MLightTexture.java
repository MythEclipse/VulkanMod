...
public class MLightTexture {
    ...
    //
    //            // TODO: Other mods might be changing lightmap behaviour, we can't be aware of
    // that here
    //
    ...
    // this.minecraft.options.darknessEffectScale().get().floatValue();
    //                float darknessGamma = this.getDarknessGamma(partialTicks) *
    // darknessEffectScale;
    //                float darknessScale = this.calculateDarknessScale(this.minecraft.player,
    // darknessGamma, partialTicks) * darknessEffectScale;
    ...
    //                } else if (waterVision > 0.0F &&
    // this.minecraft.player.hasEffect(MobEffects.CONDUIT_POWER)) {
    //                    nightVisionFactor = waterVision;
    //                } else {
    //                    nightVisionFactor = 0.0F;
    //                }
    //
    ////                Vector3f skyLightColor = new Vector3f(skyDarken, skyDarken, 1.0F).lerp(new
    // Vector3f(1.0F, 1.0F, 1.0F), 0.35F);
    ...
    //                } else {
    //                    vector3f2 = (new Vector3f(g, g, 1.0F)).lerp(new Vector3f(1.0F, 1.0F,
    // 1.0F), 0.35F);
    //                }
    //
    ...
    //                            float maxComponent = Math.max(lightColor.x(),
    // Math.max(lightColor.y(), lightColor.z()));
    ...
    //                        }
    //
    //                        tVec3f.set(this.notGamma(lightColor.x), this.notGamma(lightColor.y),
    // this.notGamma(lightColor.z));
    ...
    //
    //                        MemoryUtil.memPutInt(ptr + (((long) y * width + x) * 4L), 0xFF000000 |
    // b << 16 | g << 8 | r);
    //                    }
    //                }
    //
    //                CommandPool.CommandBuffer commandBuffer =
    // ImageUploadHelper.INSTANCE.getOrStartCommandBuffer();
    //                this.lightTexture.upload();
    //
    ...
    private float getDarknessGamma(float f) {
        ...
        return mobEffectInstance != null
                ? mobEffectInstance.getBlendFactor(this.minecraft.player, f)
                : 0.0F;
    }
    ...
    private float calculateDarknessScale(LivingEntity livingEntity, float f, float g) {
        float h = 0.45F * f;
        return Math.max(
                0.0F, Mth.cos(((float) livingEntity.tickCount - g) * (float) Math.PI * 0.025F) * h);
    }
