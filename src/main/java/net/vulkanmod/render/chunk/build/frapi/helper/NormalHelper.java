...
public abstract class NormalHelper {
    ...
    /**
     * Stores a normal plus an extra value as a quartet of signed bytes. This is the same normal
     * format that vanilla rendering expects. The extra value is for use by shaders.
     */
    ...
    public static void unpackNormal(int packedNormal, Vector3f target) {
        target.set(
                unpackNormalX(packedNormal),
                unpackNormalY(packedNormal),
                unpackNormalZ(packedNormal));
    }

    /**
     * Computes the face normal of the given quad and saves it in the provided non-null vector. If
     * {@link QuadView#nominalFace()} is set will optimize by confirming quad is parallel to that
