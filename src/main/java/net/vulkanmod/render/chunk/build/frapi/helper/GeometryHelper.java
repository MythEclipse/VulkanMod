...
/**
 * Static routines of general utility for renderer implementations. Renderers are not required to
 * use these helpers, but they were designed to be usable without the default renderer.
 */
public abstract class GeometryHelper {
     ...
     * {@link #LIGHT_FACE_FLAG} and {@link #CUBIC_FLAG}. Intended use is to optimize lighting when
     * the geometry is regular. Expects convex quads with all points co-planar.
     */
    ...
    /**
     * Returns true if quad is parallel to the given face. Does not validate quad winding order.
    ...
    public static boolean isQuadParallelToFace(Direction face, QuadView quad) {
        ...
        return equal(val, quad.posByIndex(1, i))
                && equal(val, quad.posByIndex(2, i))
                && equal(val, quad.posByIndex(3, i));
    }

    /**
     * True if quad - already known to be parallel to a face - is actually coplanar with it. For
     * compatibility with vanilla resource packs, also true if quad is outside the face.
     *
     * <p>Test will be unreliable if not already parallel, use {@link
     * #isQuadParallelToFace(Direction, QuadView)} for that purpose. Expects convex quads with all
     * points co-planar.
     */
     ...
     *
     * <p>Does not check if quad is actually coplanar with the light face, nor does it check that
     * all quad vertices are coplanar with each other.
     *
    ...
    /**
     * Used by {@link #isQuadCubic(Direction, QuadView)}. True if quad touches all four corners of
     * unit square.
     *
     * <p>For compatibility with resource packs that contain models with quads exceeding block
     * boundaries, considers corners outside the block to be at the corners.
     */
     ...
     * {@link BakedQuad#getDirection()} returns, and is used in the vanilla renderer for all diffuse
     * lighting.
     *
