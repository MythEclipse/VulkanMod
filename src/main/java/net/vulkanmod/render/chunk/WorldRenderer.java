...
public class WorldRenderer {
    ...
    private final java.util.Set<net.minecraft.world.level.block.entity.BlockEntity> globalBlockEntities = com.google.common.collect.Sets
            .newHashSet();
    ...
    public static net.vulkanmod.render.chunk.WorldRenderer init(
        ...
        net.vulkanmod.render.chunk.WorldRenderer worldRenderer = new net.vulkanmod.render.chunk.WorldRenderer(
    ...
    private void allocateIndirectBuffers() {
        ...
        this.indirectBuffers = new net.vulkanmod.vulkan.memory.buffer.IndirectBuffer[net.vulkanmod.vulkan.Renderer
                .getFramesNum()];
        for (int i = 0; i < this.indirectBuffers.length; i++) {
            this.indirectBuffers[i] = new net.vulkanmod.vulkan.memory.buffer.IndirectBuffer(
    ...
    public void setupRenderer(
        ...
        profiler.pop();
        double entityDistanceScaling = ((java.lang.Double) this.minecraft.options.entityDistanceScaling().get())
                .doubleValue();
    ...
    public void allChanged() {
        if (this.level != null) {
            ...
            this.sectionGraph = new net.vulkanmod.render.chunk.graph.SectionGraph(
    ...
    public void setLevel(
            @org.jetbrains.annotations.Nullable net.minecraft.client.multiplayer.ClientLevel level) {
    ...
    public void renderSectionLayer(
        ...
        net.vulkanmod.vulkan.shader.GraphicsPipeline pipeline = net.vulkanmod.render.PipelineManager
                .getTerrainShader(renderType);
        ...
        net.vulkanmod.render.engine.VkGpuTexture texture = (net.vulkanmod.render.engine.VkGpuTexture) gpuTexture;
        if (this.terrainSampler == 0) {
            // maxLod = 1000 (VK_LOD_CLAMP_NONE) so all mip levels are accessible
            this.terrainSampler = net.vulkanmod.vulkan.texture.SamplerManager.getSampler(true, true, 1000, false, 0);
        }
        ...
        var fadeInOption = this.minecraft.options.chunkSectionFadeInTime();
        float fadeTime = fadeInOption != null ? ((java.lang.Double) fadeInOption.get())
                .floatValue() : 0.0f;
        int fadeTimeMs = (int) (fadeTime * 1000.0f);
        float fadeTimeInv = fadeTime > 0.0f ? 1.0f / (fadeTime * 1000.0f) : 1.0f;
        net.vulkanmod.vulkan.memory.buffer.IndexBuffer indexBuffer = net.vulkanmod.vulkan.Renderer.getDrawer()
                .getQuadsIndexBuffer().getIndexBuffer();
        ...
        java.util.Set<net.vulkanmod.render.vertex.TerrainRenderType> allowedRenderTypes = net.vulkanmod.Initializer.CONFIG.uniqueOpaqueLayer
        ...
        if (allowedRenderTypes.contains(renderType)) {
            ...
            java.util.Iterator<net.vulkanmod.render.chunk.ChunkArea> iterator = this.sectionGraph.getChunkAreaQueue()
    ...
    private void sortTranslucentSections(double camX, double camY, double camZ) {
        ...
        if ((d0 * d0) + (d1 * d1) + (d2 * d2) > 2.0d) {
            ...
            java.util.Iterator<net.vulkanmod.render.chunk.RenderSection> iterator = this.sectionGraph.getSectionQueue()
                    .iterator(false);
    ...
    public void renderBlockEntities(
            ...
            it.unimi.dsi.fastutil.longs.Long2ObjectMap<java.util.SortedSet<net.minecraft.server.level.BlockDestructionProgress>> destructionProgress) {
        ...
        for (net.vulkanmod.render.chunk.RenderSection renderSection : this.sectionGraph.getBlockEntitiesSections()) {
            java.util.List<net.minecraft.world.level.block.entity.BlockEntity> list = renderSection.getCompiledSection()
                    .getBlockEntities();
            if (!list.isEmpty()) {
                for (net.minecraft.world.level.block.entity.BlockEntity blockEntity : list) {
                    ...
                    java.util.SortedSet<net.minecraft.server.level.BlockDestructionProgress> sortedSet = destructionProgress
                            .get(blockPos.asLong());
                    if (sortedSet != null && !sortedSet.isEmpty()) {
                        ...
                        crumblingOverlay = new net.minecraft.client.renderer.feature.ModelFeatureRenderer.CrumblingOverlay(
                                sortedSet.last().getProgress(), poseStack.last());
                        poseStack.popPose();
                    } else {
                        crumblingOverlay = null;
                    }
                    net.minecraft.client.renderer.blockentity.state.BlockEntityRenderState blockEntityRenderState = this.blockEntityRenderDispatcher
        ...
        java.util.Iterator<net.minecraft.world.level.block.entity.BlockEntity> iterator = this.level
                .getGloballyRenderedBlockEntities().iterator();
        while (iterator.hasNext()) {
            net.minecraft.world.level.block.entity.BlockEntity blockEntity2 = iterator.next();
            if (blockEntity2.isRemoved()) {
                iterator.remove();
            } else {
                net.minecraft.client.renderer.blockentity.state.BlockEntityRenderState blockEntityRenderState2 = this.blockEntityRenderDispatcher
                        .tryExtractRenderState(
                                blockEntity2,
                                this.partialTick,
                                null);
        ...
        for (net.minecraft.client.renderer.blockentity.state.BlockEntityRenderState blockEntityRenderState3 : levelRenderState.blockEntityRenderStates) {
            ...
            net.minecraft.client.renderer.blockentity.BlockEntityRenderDispatcher blockEntityRenderDispatcher = this.minecraft
